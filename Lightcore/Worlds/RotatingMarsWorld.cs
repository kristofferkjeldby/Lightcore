namespace Lightcore.Worlds
{
    ...
    public class RotatingMarsWorld : WorldBuilder
    {
        ...
        public override void Create(List<Entity> entities, List<Light> lights, RenderMode renderMode, int animateStep = 0)
        {
            ...
            entities.Add(Shapes.Sphere(renderMode, new Vector(0, 0, 0), 150, Map, ColorTextureStore.ColorTexture).Transform(rotateMars));

            if (Atmosphere)
            {
                entities.Add(Shapes.Sphere(renderMode, new Vector(0, 0, 0), 158, AtmosphereMap, color => new ColorTexture(color, 0.1f, 0.7f, 0.0f, 0.3f)).Transform(rotateMars));
            }
